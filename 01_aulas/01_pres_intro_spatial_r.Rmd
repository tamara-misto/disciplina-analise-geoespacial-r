---
title: "Introdução à análise geoespacial com R <br><br><br>"
subtitle: "1 Controle de versão, Git e GitHub <br><br><br>"
author: "Maurício H. Vancine <br> Milton C. Ribeiro"
date: "19/10/2020"
output:
  xaringan::moon_reader:
    css: [metropolis]
    lib_dir: libs
    nature:
      highlightStyle: rainbow
      highlightLines: true
      countIncrementalSlides: false
---

```{r setup, include=FALSE}
options(htmltools.dir.version = FALSE,
        encoding = "UTF-8")
```

class: clear, center, middle
background-image: url(img/github_octoca.jpg)
background-size: 500px

---

background-image: url(img/git_github.png)
background-size: 550px
background-position: 100% 110%

# 1 Controle de versão, Git e GitHub

## Conteúdo
1. Conferir os notebooks e PCs
1. Controle de versão
1. Git
1. GitHub
1. Funcionamento do controle de versão
1. Fork
1. Iniciando: init ou clone
1. Configurando: config
1. Básico: add, status, commit, log e reset
1. Ramificações: branch, checkout e merge
1. Remoto: fetch, push, pull
1. Pull request

---

class: inverse, middle, center

# Vamos conferir os notebooks?!

---

background-image: url("img/general_hard_soft.png")
background-size: 700px
background-position: 50% 50%

# Software e Hardware

---

background-image: url(img/r_logo.svg)
background-size: 400px
background-position: 50% 70%

# 1.1 Verificar os notebooks

## R (>4.0.x)

### https://www.r-project.org/

---

background-image: url(img/rstudio_logo.svg)
background-size: 600px
background-position: 50% 60%

# 1.1 Verificar os notebooks

## RStudio (>1.3.x)

### https://www.rstudio.com/

---

background-image: url(img/git_logo.png)
background-size: 500px
background-position: 50% 60%

# 1.1 Verificar os notebooks

## git (>2.28)

### https://git-scm.com/

---

class: inverse, middle, center

# Tudo instalado? Então bora!

---

background-image: url(img/git_file_version.png)
background-size: 400px
background-position: 50% 50%

# 1.2 Controle de versão

---

background-image: url(img/git_one_brantch.png)
background-size: 600px
background-position: 50% 50%

# 1.2 Controle de versão

## Manejar projetos **individualmente**

---

background-image: url(img/git_multi_brantch.png)
background-size: 700px
background-position: 50% 50%

# 1.2 Controle de versão

## Manejar projetos **compartilhados**

---

background-image: url(img/git_logo.png)
background-size: 450px
background-position: 50% 70%

# 1.3 Git

## Software que faz o **controle de versão**

### Maneja os repositórios locais (notebook) e remotos (GitHub)
 
---

background-image: url(img/github_logo.png)
background-size: 400px
background-position: 50% 75%

# 1.4 GitHub

## Repositório remoto

### **Plataforma de hospedagem** de código-fonte com controle de versão usando o Git

---

background-image: url(img/git_github_diff.png)
background-size: 800px
background-position: 50% 80%

# ATENÇÃO!

### **Git**: software que faz o controle de versão

### **GitHub**: repositório remoto que hospeda os arquivos versionados

---

background-image: url(img/git_github_link.png)
background-size: 800px
background-position: 50% 80%

# ATENÇÃO!

### **Git**: software que faz o controle de versão

### **GitHub**: repositório remoto que hospeda os arquivos versionados

---

# 1.4 GitHub

## Vamos **criar uma conta** no **GitHub** (caso não possuam)
<br><br><br><br><br>
##.center[https://github.com]

---

# 1.4 GitHub

## **Salvem ou recuperem essas informações!!!**

<br><br><br>

## **username**: mauriciovancine
## **email**: mauricio.vancine@gmail.com
## **senha**: !@#$%¨&*+

---

class: inverse, middle, center

# Tudo bem até aqui?

---

background-image: url(img/git_fork_clone_flow.png)
background-size: 450px
background-position: 50% 50%

# 1.5 Funcionamento do controle de versão

---

class: clear, center, middle
background-image: url(img/git_work.png)
background-size: 400px

---

background-image: url(img/git_fork.png)
background-size: 500px
background-position: 50% 60%

# 1.6 Fork

## **Fork**: **copia** um repositório remoto para o **seu GitHub**

---

background-image: url(img/github_fork_site.png)
background-size: 800px
background-position: 50% 70%

# 1.6 Fork

## Forken o repositório da disciplina para o GitHub de vocês

###.center[https://github.com/mauriciovancine/disciplina-analise-geoespacial-r]

---

background-image: url(img/gif_fork.gif)
background-size: 600px
background-position: 50% 80%

# 1.6 Fork

## Forken o repositório da disciplina para o GitHub de vocês

###.center[https://github.com/mauriciovancine/disciplina-analise-geoespacial-r]

---

background-image: url(img/git_init.png)
background-size: 400px
background-position: 50% 80%

# 1.7 Iniciando: clone ou init

## **init**: **inicia o versionamento** de um repositório

---

background-image: url(img/git_clone.png)
background-size: 500px
background-position: 50% 80%

# 1.7 Iniciando: clone ou init

## **clone**: **copia** um repositório remoto para o **seu PC**

---

background-image: url(img/git_clone.png)
background-size: 500px
background-position: 50% 80%

# 1.7 Iniciando: clone ou init

## Vamos clonar o **repositório remoto (GitHub)** para o **repositório local (PC)**

---

background-image: url(img/general_directory.png)
background-size: 350px
background-position: 50% 90%

# 1.7 Iniciando: clone ou init

## **Criem uma pasta** chamada `github` no PC de vocês

```{r eval=FALSE}
".                               # raiz"
"    ├── home/                   # home"
"        ├── data/               # dados" 
"            ├── github/         # todos os repositorios"
```

---

class: inverse, middle, center

# E como faremos isso?

---

background-image: url(img/github_clone_zip.png)
background-size: 800px
background-position: 50% 60%

# 1.7 Iniciando: clone ou init

## 1. **Download diretamente** do repositório remoto no formato **.zip**

---

background-image: url(img/github_clone_git.png)
background-size: 800px
background-position: 50% 60%

# 1.7 Iniciando: clone ou init

## 2. Usar o **terminal** para clonar pelo **HTTPS do repositório**

---

background-image: url(img/git_rstudio.jpg)
background-size: 500px
background-position: 50% 80%

# 1.7 Iniciando: clone ou init

## 3. Usar o **RStudio** para clonar pelo **HTTPS do repositório**

---

class: inverse, middle, center

# Vamos abrir o RStudio

---

background-image: url(img/rstudio.png)
background-size: 800px
background-position: 50% 50%

# RStudio

---

background-image: url(img/git_rstudio_01.png)
background-size: 750px
background-position: 50% 100%

# Git e RStudio

## Indicar o caminho do git para o RStudio

- `Tools > Globla Options > Git/SVN`

---

background-image: url(img/git_rstudio_02.png)
background-size: 450px
background-position: 70% 85%

# Git e RStudio

## Marcar **Enable version...** e **Git executable**:

- Windows: `C:/Program Files/Git/bin/git.exe`
- Ubuntu: `/usr/bin/git`

---

background-image: url(img/git_rstudio_create_ssh_key.jpg)
background-size: 500px
background-position: 80% 85%

# Git e RStudio

## Criar uma chave SSH

- `Create RSA key`

- `View public key`

- `Copiar: Ctrl + C`

---

background-image: url(img/github_ssh_key.png)
background-size: 900px
background-position: 50% 70%

# GitHub e RStudio

## No GitHub...

- `Perfil > Settings > SSH and GPG keys > New SSH key`

---

background-image: url(img/github_ssh_key_add.png)
background-size: 900px
background-position: 50% 80%

# GitHub e RStudio

## No GitHub...

- `Title: um nome qualquer`
- `Key: colar (Ctrl + V)`
- `Add SSH key`

---

class: inverse, middle, center

# Tudo certo até aqui?

---

background-image: url(img/rstudio_git_01_newproj.png)
background-size: 800px
background-position: 50% 80%

# 1.7 Iniciando: clone ou init

## Criar um **Projeto R** com controle de versão

`File > New Project > Version Control`

---

background-image: url(img/rstudio_git_02_choosegit.png)
background-size: 800px
background-position: 50% 80%

# 1.7 Iniciando: clone ou init

## Escolher **clonar repositório** do GitHub

`Git`

---

background-image: url(img/rstudio_git_03_fillrepo.png)
background-size: 750px
background-position: 50% 80%

# 1.7 Iniciando: clone ou init

## Preencher:

Repository URL: https://github.com/mauriciovancine/disciplina-analise-geoespacial-r.git

Create project as subdirectory of: /home/mude/data/github

---

background-image: url(img/general_terminal.webp)
background-size: 300px
background-position: 50% 90%

# 1.7 Iniciando: clone ou init

## Terminal

### Na aba **Terminal** do RStudio, digite:

```{bash eval=FALSE}
# terminal
git clone https://github.com/mauriciovancine/disciplina-analise-geoespacial-r.git
```

---

background-image: url(img/git_config.jpg)
background-size: 450px
background-position: 50% 90%

# 1.8 Configurando: config

## Precisamos **definir as configurações** do git

```{bash eval=FALSE}
# terminal
git config --list
git config --global user.name "meu nome"
git config --global user.email "email@dominio.com"
git config --list
```

---

background-image: url(img/git_add_commit.png)
background-size: 800px
background-position: 50% 50%

# Fluxo de trabalho com o git e GitHub

---

background-image: url(img/git_workflow.svg)
background-size: 800px
background-position: 50% 50%

# Fluxo de trabalho com o git e GitHub

## Todos os **comandos em SHELL** para usar o git

---

background-image: url(img/general_bloco_notas.png)
background-size: 250px
background-position: 50% 80%

# Vamos criar um arquivo no diretório

## Usando a aba **Terminal** do RStudio

```{bash eval=FALSE}
# terminal
cat > arquivo_teste.txt
teste
# Ctrl + D - fechar
```

---

background-image: url(img/git_state_files.png)
background-size: 550px
background-position: 50% 90%

# Git Status dos arquivos

## Estados de um arquivo:

1. **Untracked**: não registrou o arquivo criado
1. **Tracked**: registrou o arquivo criado
1. **Staged**: os dados são armazenados com segurança em seu banco de dados local
1. **Committed**: os dados são armazenados com segurança em seu banco de dados local
1. **Modified**: os dados são armazenados com segurança em seu banco de dados local

---

background-image: url(img/git_states.png)
background-size: 800px
background-position: 50% 90%

# Git Status

## Estados do repositório local do git:

1. **Modified**: você alterou os arquivos
1. **Staged**: você marcou um arquivo modificado para ir para o commit
1. **Committed**: os dados são armazenados com segurança em seu banco de dados local (.git)

---

background-image: url(img/git_add_commit_flow.png)
background-size: 700px
background-position: 50% 90%

# Git Sections

## Seções de um repositório git:

1. **Working directory**: 
1. **Staging Area**: 
1. **Repository**: 

---

background-image: url(img/git_add_commit_flow.png)
background-size: 600px
background-position: 50% 70%

# 1.9 Básico: status, add, commit, log e reset

## **status**: **mostra** o status do repositório e dos arquivos

```{bash eval=FALSE}
```

---

background-image: url(img/git_add_commit_flow.png)
background-size: 600px
background-position: 50% 70%

# 1.9 Básico: status, add, commit, log e reset

## **add**: **adiciona** mudanças após edições (staging area)

```{bash eval=FALSE}
# terminal
git add -Av
```

---

background-image: url(img/git_add_commit_flow.png)
background-size: 600px
background-position: 50% 70%

# 1.9 Básico: status, add, commit, log e reset

## **status**: **mostra** o estatus do repositório

```{bash eval=FALSE}
```

---

background-image: url(img/git_add_commit_flow.png)
background-size: 600px
background-position: 50% 70%

# 1.9 Básico: status, add, commit, log e reset

## **commit**: **armazena as mudanças** efetuadas e **descreve** o que foi alterado (repository)

```{bash eval=FALSE}
```

---

background-image: url(img/git_add_commit_flow.png)
background-size: 600px
background-position: 50% 70%

# 1.9 Básico: status, add, commit, log e reset

## **status**: **mostra** o estatus do repositório

```{bash eval=FALSE}
```

---

background-image: url(img/git_add_commit.png)
background-size: 500px
background-position: 50% 90%

# 1.9 Básico: status, add, commit, log e reset

## **log**: **mostra as mudanças** que foram efetuadas: commit, autor e data

```{bash eval=FALSE}
```

---

background-image: url(img/git_add_commit.png)
background-size: 500px
background-position: 50% 90%

# 1.9 Básico: status, add, commit, log e reset

## **reset**: **desfazer alterações** no diretório

```{bash eval=FALSE}
```

---

background-image: url(img/git_add_commit.png)
background-size: 500px
background-position: 50% 90%

# 1.9 Básico: status, add, commit, log e reset

## **reset --soft**: **desfazer alterações** no diretório

```{bash eval=FALSE}
```

---

background-image: url(img/git_add_commit.png)
background-size: 500px
background-position: 50% 90%

# 1.9 Básico: status, add, commit, log e reset

## **reset --mixed**: **desfazer alterações** no diretório

```{bash eval=FALSE}
```

---

background-image: url(img/git_add_commit.png)
background-size: 500px
background-position: 50% 90%

# 1.9 Básico: status, add, commit, log e reset

## **reset --hard**: **desfazer alterações** no diretório

```{bash eval=FALSE}
```

---

background-image: url(img/git_branch)
background-size: 500px
background-position: 50% 90%

# 1.10 Ramificações: branch, checkout e merge

## branch: Enviar uma nova versão do **repositório local** para o **repositório remoto (GitHub)**

---

background-image: url(img/git_branch)
background-size: 500px
background-position: 50% 90%

# 1.10 Ramificações: branch, checkout e merge

## checkout: Enviar uma nova versão do **repositório local** para o **repositório remoto (GitHub)**

---

background-image: url(img/git_branch)
background-size: 500px
background-position: 50% 90%

# 1.10 Ramificações: branch, checkout e merge

## merge: Enviar uma nova versão do **repositório local** para o **repositório remoto (GitHub)**

---

background-image: url(img/git_branch)
background-size: 500px
background-position: 50% 90%

# 1.11 Remoto: fetch, push, pull

## fetch: enviar uma nova versão do **repositório remoto (GitHub)** para o **repositório local**

---

background-image: url(img/git_branch)
background-size: 500px
background-position: 50% 90%

# 1.11 Remoto: fetch, push, pull

## push: enviar uma nova versão do **repositório remoto (GitHub)** para o **repositório local**

---

background-image: url(img/git_branch)
background-size: 500px
background-position: 50% 90%

# 1.11 Remoto: fetch, push, pull

## pull: enviar uma nova versão do **repositório remoto (GitHub)** para o **repositório local**

---

background-image: url(https://diegopftrindade.netlify.com/img/8verscontrol.png)
background-size: 800px
background-position: 50% 60%

# 1.12 Pull request

## Pull request: 

---

background-image: url(https://diegopftrindade.netlify.com/img/9changes.png)
background-size: 800px
background-position: 50% 60%

# 1.10 Controle de versão no RStudio

## RStudio

---

background-image: url(https://diegopftrindade.netlify.com/img/commit.png)
background-size: 800px
background-position: 50% 60%

# 1.10 Controle de versão no RStudio

## RStudio

---

background-image: url(img/cheatsheet_git.jpg)
background-size: 350px
background-position: 50% 50%

# Mais informações

## Git Cheat Sheets

<br><br><br><br><br><br><br><br><br><br><br><br><br><br>

[*] https://github.github.com/training-kit/downloads/pt_BR/github-git-cheat-sheet.pdf

---

background-image: url(img/package_usethis.png)
background-size: 200px
background-position: 85% 65%

# Mais informações

## Pacote [usethis](https://usethis.r-lib.org/)

#### Comandos diretos no R para versionar um repositório

```{r,eval=FALSE}
install.packages("usethis")
devtools::install_github("r-lib/usethis")
```

<br><br><br><br><br>

## Material

#### [Happy Git and GitHub for the useR](https://happygitwithr.com/) - Jenny Bryan

#### [Primeiros passos utilizando o Git e GitHub no RStudio](https://beatrizmilz.github.io/RLadies-Git-RStudio-2019) - Beatriz Milz

---

class: inverse, middle, center

# Dúvidas?

---

class: clear, middle

## Maurício Vancine

<br>

Contatos:

<br>
`r icon::fa_envelope(colour = "#007D8D")` [mauricio.vancine@gmail.com]()
<br>
`r icon::fa_twitter(colour = "#007D8D")` [@mauriciovancine](https://twitter.com/mauriciovancine)
<br>
`r icon::fa_github(colour = "#007D8D")` [mauriciovancine](https://mauriciovancine.netlify.com/)
<br>
`r icon::fa_link(colour = "#007D8D")` [mauriciovancine.netlify.com](https://mauriciovancine.netlify.com/)

Slides criados via pacote [xaringan](https://github.com/yihui/xaringan) e tema [Metropolis](https://github.com/pat-s/xaringan-metropolis)