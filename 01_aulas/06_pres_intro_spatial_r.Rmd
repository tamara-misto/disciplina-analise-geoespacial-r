...
```{r}
# convert datum
rc_2019_utm_wgs84 <- sf::st_transform(sp_mun_2019, crs = 32723)
rc_2019_utm_wgs84
```
...
```{r fig.align='center', out.width='50%'}
# plot
plot(rc_use_forest$geometry, col = "forestgreen", main = NA, axes = TRUE, graticule = TRUE)
```
...
## 3. Aggregation de atributos
```{r}
plot(rc_spr_n$geometry, pch = 20, col = "blue", main = "")
```
...
```{r echo=FALSE, fig.align='center', out.width='50%'}
# plot
plot(sp_mun_2019_simp$geom, col = "gray", main = NA)
```
...
plot(sp_mun_2019$geom, main = NA)
plot(sp_mun_2019_cent$geom, col = "red", pch = 20, main = NA, add = TRUE)
plot(sp_mun_2019_cent_buf$geom, col = "gray", main = NA, add = TRUE)
```
